...
`parse_instr_name`, `parse_str_num` and `parse_reg` parses an input string into
a type usable by Jas.

Jas' instruction encoder reference table provides a list of instructions in a
string format. The `parse` module function enables seamless integration with the
supported instructions of the Jas instruction encoder bank whilst interacting
with application code for the compilation of full instructions.

### Synopsis

_The three functions' signatures are listed below:_
...
> may be obtained in the _instruction_ module
...
placeholder values for their respective types.

> [!WARNING]
> A return value of `INSTR_NULL` or `REG_NULL`, or any type's _null_ placeholder
> value indicates an error during the process and should be handled accordingly.

Due to `parse_str_num`'s dependence on the standard library's `strtoull`
function, the value dictating a failure would default to `0` where an upstream
error has occurred during the execution of an operating system dependent
function such as `strtoull`.
