...
> [!TIP]
> `op_assert_types` is tightly integrated with the `op_descriptor_t` used in
> conjunction as seen in the `instr_encode_tab` structure, supporting for error
...
`op_assert_descriptor` returns a boolean value indicative of whether the
assertion is successful.

A `true` boolean value is returned if the requirements expressed by operand
descriptors in `expected` are reflected as per `in`. Conversely, if the
conditions are not met, `false` is to be returned, prompting appropriate error
reporting measures.
